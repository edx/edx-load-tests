...
class DiscussionsSeeder(object):
    ...
    def seed_comments(self, course_id, posts):
        """
        ...
        for i in range(0, posts):
            ...
            comment_id = self.create_comment(big_thread)

            for j in range(0, ((i + 1) * 20)):
                self.create_comment(big_thread, comment_is_child=comment_id)
            print "Created thread {} of {} with {} comments".format(i, posts, str((i + 1) * 20))
    ...
    def create_thread_data_file(self, file_name, course_id):
        """
